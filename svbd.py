...
def correct_name(video_name):
    ...
    cleaned_filename = unicodedata.normalize('NFKD', video_name).encode('ASCII', 'ignore').decode('ASCII')
    return ''.join(c for c in cleaned_filename if c in valid_chars)


def main():
    argpars = argparse.ArgumentParser(description='Safari video book downloader.')
    argpars.add_argument('-u', '--url', required=True, dest='url', help='URL to download')
    argpars.add_argument('-l', '--login', required=True, dest='login')
    argpars.add_argument('-p', '--password', required=True, action=Password, nargs='?', dest='password', help='Enter your password')
    argpars.add_argument('-d', '--dir', dest='directory', help='Directory to save content')
    ...
    # self.title = self.title[0].text.strip()
    ...
    for topic in contents:
        ...
        book_dir = '{}/{}/{}'.format(parrent_dir, title, topic_name)
        ...
        for index, video in enumerate(topic.ol.find_all('a'), start=1):
            ...
            subprocess.run([DOWNLOADER, "-u", args.login, "-p", args.password, "--output", video_f2s, video_url])
